...
describe("CreateGroupRequestDto", () => {
  it("is invalid when name is missing", async () => {
		const dto = plainToInstance(CreateGroupRequestDto, {
      type: "trip",
      currency: "ARS",
    });
  ...
  it("allows omitting members or sending an empty array", async () => {
		const withoutMembers = plainToInstance(CreateGroupRequestDto, {
      name: "Trip to Bariloche",
      type: "trip",
      currency: "ARS",
    });
		const withEmptyMembers = plainToInstance(CreateGroupRequestDto, {
      name: "Trip to Bariloche",
      type: "trip",
      currency: "ARS",
      members: [],
